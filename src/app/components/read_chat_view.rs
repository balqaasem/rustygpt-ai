...
const READ_CHAT_VIEW_CLASS: &str = "h-screen pb-24 w-full flex flex-col overflow-y-auto p-5";
const READ_CHAT_VIEW_LIGHT_MODE_COLORS: &str = "border-gray-300 bg-gray-100";
const READ_CHAT_VIEW_DARK_MODE_COLORS: &str = "border-zinc-700 bg-zinc-900";

#[component]
pub fn ReadChatView(conversation: ReadSignal<Conversation>) -> impl IntoView {
  ...
  let read_chat_view_class = Signal::derive(move || {
    if dark_mode.get() {
      format!("{READ_CHAT_VIEW_CLASS} {READ_CHAT_VIEW_DARK_MODE_COLORS}")
    } else {
      format!("{READ_CHAT_VIEW_CLASS} {READ_CHAT_VIEW_LIGHT_MODE_COLORS}")
    }
  });

  view! {
        <div class={read_chat_view_class.get()} node_ref=chat_div_ref>
